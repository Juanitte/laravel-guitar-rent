	...
	<table class="text-left w-full">
        ...
        <tbody class="bg-grey-light flex flex-col items-center justify-between overflow-y-scroll w-full" style="height: 50vh;">
			@foreach ($clients as $client)
                <tr class="flex w-full mb-4">
                    <td class="p-4 w-1/4">{{$client->name}}</td>
                    <td class="p-4 w-1/4">{{$client->email}}</td>
                    <td class="p-4 w-1/4 flex flex-row">
                        <a class="hover:cursor-pointer pr-2" wire:click="openCreateModal({{$client}}, false)">
                        ...
                        <a class="hover:cursor-pointer px-2" wire:click="openCreateModal({{$client}})">
                        ...
                        <a class="hover:cursor-pointer pl-2" wire:click="deleteClient({{$client}})">
        ...
        <div class="max-h-full w-full max-w-xl overflow-y-auto sm:rounded-2xl bg-white">
          <div class="w-full">
            <div class="m-8 my-20 max-w-[400px] mx-auto">
                @if ($isEditing)
                    <h1 class="text-3xl font-bold text-center mb-4">{{isset($myClient->id) ? 'Edit' : 'Create new'}} client</h1>
                @else
                    <h1 class="text-3xl font-bold text-center mb-4">{{$myClient->name}} info</h1>
                @endif
                <div class="mb-4">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Name</label>
                    @if ($isEditing)
                        <input type="text" id="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-800 focus:border-gray-800 block w-full p-2.5" wire:model="name">
                    @else
                        <p>{{$myClient->name}}</p>
                    @endif
                </div>
                <div class="mb-4">
                    ...
                    @if ($isEditing)
                        <input type="text" id="surname" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-800 focus:border-gray-800 block w-full p-2.5" wire:model="surname">
                    @else
                ...
                <div class="mb-4">
                    ...
                    @if ($isEditing)
                        <input type="text" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-800 focus:border-gray-800 block w-full p-2.5" wire:model="email">
                    @else
                        <p>{{$myClient->email}}</p>
                    @endif
                </div>
                <div class="mb-4">
                    ...
                    @if ($isEditing)
                        <input type="text" id="phone" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-gray-800 focus:border-gray-800 block w-full p-2.5" wire:model="phone">
                    @else
                        <p>{{$myClient->phone}}</p>
                    @endif
                </div>
                <div class="flex flex-row">
                    @if ($isEditing)
                        <button class="p-3 bg-gray-800 rounded-xl text-white w-full font-semibold" wire:click="createOrUpdateClient">
                            {{isset($myClient->id) ? 'Update' : 'Create'}} Client
                        </button>
                    @endif
